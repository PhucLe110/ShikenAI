...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [{ data: userData }, { data: submissionsData }] =
          await Promise.all([
  ...
  const handleUnlockRequest = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/users/request-unlock", {
      ...
      <div className={styles.lockedContainer}>
        <div className={styles.lockedCard}>
          ...
          <p className={styles.lockedReason}>
            <strong>Lý do:</strong>{" "}
            {user.lockReason || "Vi phạm điều khoản cộng đồng"}
          </p>
          ...
          <div className={styles.unlockSection}>
            ...
            {user.unlockRequest?.status === "rejected" && (
              <div className={styles.rejectedBox}>
                ❌ Yêu cầu trước đó đã bị từ chối. Bạn có thể gửi lại yêu cầu
                mới nếu có thêm bằng chứng.
              </div>
            )}
          </div>
    ...
    <div className={styles.container}>
      ...
      <div className={styles.stats}>
        ...
        <div className={`${styles.statCard} ${styles.statCardInProgress}`}>
          <span className={`${styles.statValue} ${styles.statValueInProgress}`}>
            {inProgress.length}
          </span>
      ...
      <div className={styles.tabs}>
        ...
        >
          ⏸ Chưa hoàn thành
          {inProgress.length > 0 && (
            <span className={`${styles.tabBadge} ${styles.tabBadgeOrange}`}>
              {inProgress.length}
            </span>
          )}
        </button>
            ...
            <div className={styles.historyList}>
              {completed.map((sub) => (
                <div key={sub._id} className={styles.historyCard}>
                  ...
                  <span
                    className={`${styles.scoreChip} ${sub.score >= 50 ? styles.passed : styles.failed}`}
                  >
            ...
            <div className={styles.historyList}>
              {inProgress.map((sub) => {
                ...
                return (
                  <div
                    key={sub._id}
                    className={`${styles.historyCard} ${styles.historyCardInProgress}`}
                  >
                    <div
                      className={`${styles.levelBadge} ${styles.levelBadgeInProgress}`}
                    >
                      {sub.examId?.level || "N?"}
                    </div>
                    <div className={styles.historyInfo}>
                      ...
                      <span className={styles.inProgressText}>
                        {answeredCount}/{sub.totalQuestions} câu đã trả lời (
                        {progressPct}%)
                      </span>
                    </div>
                    <button
